...
checkAndFilterSNPcoord <- function(user_SNPcoord, vcf_SNPcoord) {
  ...
  if (length(additionalSNP) != 0) {
    msg <- paste(
      "The SNPs coordinate file have", length(additionalSNP),
  ...
  if (any(is.na(user_SNPcoord$physPos))) {
    ...
    msg <- paste(
      "Some SNP physical position was not provided in SNPcoord file.",
  ...
  if (!any(is.na(vcf_SNPcoord$physPos))) {
    if (!isTRUE(all.equal(
      user_SNPcoord[row.names(user_SNPcoord), "physPos"],
...
check_fixed <- function(fixed, test) {
  ...
  if (test %in% c("wald", "lrt")) {
    ...
    warning(
      '`fixed` seems to have been set but it will not be taken in account when `test` is "',
      test, '"'
    )
  }
  invisible(TRUE)
  ...
  if (!is.null(geno)) {
    if (!any(geno@snps$maf > thresh_maf)) {
      engineError("The provided `thresh_maf` is too high and filter out all SNPs.",
        n_skip_caller = 3,
        extra = list(
          code = errorCode("BAD_ARG_THRESH_MAF_FILTER_OUT_ALL_MARKERS"),
          provided = thresh_maf,
  ...
  if (!is.null(geno)) {
    if (!any(geno@snps$callrate > thresh_callrate)) {
      engineError("The provided `thresh_callrate` is too high and filter out all SNPs.",
        n_skip_caller = 3,
        extra = list(
          code = errorCode("BAD_ARG_THRESH_CALLRATE_FILTER_OUT_ALL_MARKERS"),
...
check_method <- function(method, tau, omega) {
  code <- "BAD_ARG_METHOD"
  if (length(method) != 1) {
    bad_argument("length(method)",
      must_be = "1", not = length(method),
      n_skip_caller = 3,
      extra = list(code = errorCode(code))
    )
  }
  ...
  if (method != "Martini") {
    if (!is.null(tau) || !is.null(omega)) {
      warnMsg <- paste(
        '`tau` and `omega` are only used for "Martini" method.',
