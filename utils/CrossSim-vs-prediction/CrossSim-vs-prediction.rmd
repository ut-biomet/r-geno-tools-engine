...
idLine <- data.frame(
  mean = c(
    min(c(pred_vs_sim$blup_exp, pred_vs_sim$obs_mean)) - 1,
    max(c(pred_vs_sim$blup_exp, pred_vs_sim$obs_mean)) + 1
  ),
  var = c(
    min(c(pred_vs_sim$blup_var, pred_vs_sim$obs_var)) - 1,
...
p <- plotBlup_2traits(
  blupDta = blup_estimations[names(blup_estimations) %in% cross_to_keep],
  x_trait = "trait1",
  y_trait = "trait2",
  confidenceLevel = 0.95
)
